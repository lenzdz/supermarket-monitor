...
from scraper.makro_api import info_producto_makro
...
from notifiers.discord import enviar_mensaje_canal_makro
...
def monitor_makro():
    ...
    with open("data/productos_makro.json", encoding="utf-8") as archivo:
        productos_makro = json.load(archivo)

    enviar_mensaje_canal_makro(f"🎉 Productos en oferta ahora ({fecha_hoy}) 🎉")

    counter = 0
    for producto in productos_makro:
        ...
        resultado = revisar_producto_makro(url_producto)
        ...
        if resultado:
            ...
            enviar_mensaje_canal_makro(mensaje)
    ...
    enviar_mensaje_canal_makro(mensaje_final)

def revisar_producto_makro(url_producto):

    datos_producto = info_producto_makro(url_producto)
